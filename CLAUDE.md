...
Public uptime status page for every app in the `*.lampham.space` suite, plus their version and last-commit info. Built with **Astro (SSR, server output)** + Tailwind CSS v4 + the suite design system (`data-app="status"`, teal accent) — the first app to adopt it and its reference implementation, see [`../docs/DESIGN.md`](../docs/DESIGN.md). No Preact islands (no interactivity needed), but it **does** carry the shared auth pattern now — see "Login-gated visibility" below.
...
src/
  ...
  styles/
    global.css                  # imports ./lampham (design system) + the history-strip bars
    lampham/                    # GENERATED by ../scripts/sync-design.sh — don't edit here

  layouts/
    BaseLayout.astro            # data-app="status", SuiteHead, suite bar (login/logout via
                                  # authHref), <main>, suite footer

  components/
    lampham/                     # GENERATED: SuiteHead/SuiteHeader/SuiteFooter + suite.ts
    AppCard.astro                 # one monitored entry's lp-card — dot, name, version chip, uptime,
                                   # 48-bar history strip, last-commit line. Shared by both page
                                   # sections so the Apps/Services split doesn't duplicate the template.

  pages/
    index.astro                  # summary headline ("All systems operational" / "N down", over what
                                   # the viewer can see) + Apps and Services sections, filtered by
                                   # public/loggedIn/authUp
```
